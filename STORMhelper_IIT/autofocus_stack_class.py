...
class autofocus_stack():
    ...
    def __init__(self):
        ...
        #self.y0, self.x0 = self.calc_centroid(self.cent_image_data, self.x0, self.y0)
    ...
    def camera_setup(self):
        ...
        self.cam.set_aeag_roi_width(107)
        self.cam.set_aeag_roi_height(100)
        self.cam.set_aeag_roi_offset_x(630)
        self.cam.set_aeag_roi_offset_y(470)
    ...
    def calc_radius(self, image_data, image_width, image_height, testing_):
        #image_width = 0
        #image_height = 0
        #img = xiapi.Image()
        ...
        if testing_ == 1:
            ...
            plt.scatter(self.x0,self.y0, color = 'r')
            for i in range(len(ellipse_x)):
                plt.scatter(ellipse_x[i],ellipse_y[i], color = 'b')
            plt.show()
        ...
        print thresh_mean_left, thresh_mean_right
        if left_sum > right_sum:
    ...
    def main(self):
        loaded_array = np.load('C:/Users/STORM/Documents/autofocus/storm_numpy_stacks/fix_centroid_problem.npy')
        image_width = np.size(loaded_array[1])/len(loaded_array[1])
        image_height = len(loaded_array[1])
        print(len(loaded_array))
        z_stage_positions = np.arange(-20.0, 20.0, 0.25)
        rad_list = []
        x_cent_list = []
        y_cent_list = []
        x_cent_list1 = []
        y_cent_list1 = []
        testing_ = 0
        for image in loaded_array[::1]:
            #x_centre1, y_centre1 = self.calc_centroid(image, self.x0, self.y0)
            y_centre, x_centre = ndimage.measurements.center_of_mass(image)
            rad_ = self.calc_radius(image, image_width, image_height, testing_)
            y_centre1, x_centre1 = ndimage.measurements.center_of_mass(rad_[1])
            #print x_centre, y_centre, x_centre1, y_centre1
            rad_list.append(rad_)
            x_cent_list.append(x_centre)
            y_cent_list.append(y_centre)
            x_cent_list1.append(x_centre1)
            y_cent_list1.append(y_centre1)
        return z_stage_positions, rad_list, x_cent_list, y_cent_list, x_cent_list1, y_cent_list1

    
z_list, radii_list, x_cent_list, y_cent_list, x_cent_list1, y_cent_list1 = autofocus_stack().main()
plt.figure('radii')
for i in range(len(radii_list)):
    plt.scatter(z_list[i], radii_list[i][0])

plt.figure('raw x cent')
for i in range(len(radii_list)):
    plt.scatter(z_list[i], x_cent_list[i])

plt.figure('raw y cent')
for i in range(len(radii_list)):
    plt.scatter(z_list[i], y_cent_list[i])

plt.figure('processed x cent')
for i in range(len(radii_list)):
    plt.scatter(z_list[i], x_cent_list1[i])

plt.figure('processed y cent')
for i in range(len(radii_list)):
    plt.scatter(z_list[i], y_cent_list1[i])
